...
class TiposNormasManager:
    ...
    def add_batch(self, tipos: List[Dict]) -> Dict:
        ...
        try:
            ...
            # Preparar datos
            data = [
                (tipo["id"], tipo["nombre"], tipo.get("abreviatura"), datetime.now())
                for tipo in tipos
            ]
            ...
            self.conn.commit()
            print(
                f"\tBatch completado: {stats['insertados']} nuevos, {stats['actualizados']} actualizados"
            )
    ...
    def get_or_create(
        self, id: int, nombre: str, abreviatura: Optional[str] = None
    ) -> Dict:
